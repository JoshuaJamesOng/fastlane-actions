module Fastlane
  module Actions
    ...
    class CreateJiraSubIssueAction < Action
      def self.run(params)
        UI.important('Creating a JIRA sub-issue')
        ...
        case response[:status]
          when 201
            UI.success('Successfully created JIRA sub-issue')
      ...
      def self.create_sub_issue(server_url, api_token, certificate, body)
      ...
      def self.description
        'This will create a new JIRA sub-issue for the given project and parent'
      end

      def self.details
        [
            'Creates a new JIRA sub-issue for the given project and parent.',
            '',
            'You must provide a username & password in the format `username:password`, the server url, the project id, the parent id, a type id, and a issue summary and description'
        ].join("\n")
      end
      ...
      def self.available_options
        [
            ...
            FastlaneCore::ConfigItem.new(key: :parent_id,
                                         ...
                                         optional: false,
                                         verify_block: proc do |value|
                                           UI.user_error!("No parent id, pass using `parent_id: 'INTEGER'`") unless (value and not value.empty?)
                                         end),
